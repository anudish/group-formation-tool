...
public class CourseDAO implements ICourseDAO {
    ...
    public void enrollStudentToCourse(Student studentDetails, String courseId) {
        ...
        try {
            connection = ObtainDataBaseConnection.obtainDatabaseConnection();
            query = "insert into student_enrollments (BANNER_ID,MAIL_ID,COURSE_ID) values ('" + studentDetails.getBannerId() + "','" + studentDetails.getEmail() + "','" + courseId + "')";
            statement = connection.prepareStatement(query);
            queryResult = statement.executeUpdate();
            System.out.println(queryResult);

            query = "insert into USER_DATABASE (MAIL_ID,FIRST_NAME,LAST_NAME,ROLE) values ('" + studentDetails.getEmail() + "','" + studentDetails.getFirstName() + "','" + studentDetails.getLastName() + "','Student')";
            statement = connection.prepareStatement(query);
            queryResult = statement.executeUpdate();
            System.out.println(queryResult);

            query = "insert into AUTHENTICATION_DATABASE (MAIL_ID,PASSWORD) values ('" + studentDetails.getEmail() + "','" + studentDetails.getEncryptedPassword() + "')";
            statement = connection.prepareStatement(query);
            queryResult = statement.executeUpdate();
            System.out.println(queryResult);
